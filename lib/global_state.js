...
GlobalState.prototype = {
	...
	topic_callback: function topic_callback(full_topic_name, data) {
		...
		if(type == "event") {
			try {
				...
				console.log("Received publish for unknown topic:event");
			}
		}
		else if(type == "user") {
			...
			if(this.cstate_by_guid[guid]) {
				...
				console.log("Received publish for unknown topic:user");
			}
			 ...
			 */
			try {
				if(obj["event"] == "user_joined_event") {
					sys.debug("Adding User " + state.guid + " to event " + obj["params"]["event_guid"]);
					this.add_client_to_topic(obj["params"]["event_guid"], state);
	...
	add_client_state: function add_client_state(client_state) {
		...
		var self = this;
		pubsub_client.subscribeTo("topic:user:" + client_state.guid, function(){self.topic_callback.apply(self, arguments)} );
	...
	add_client_to_topic: function add_client_to_topic(topic, client_state) {
		...
		if(!this.cstate_by_topic[topic]) {
			...
			pubsub_client.subscribeTo("topic:event:" + topic, function(){self.topic_callback.apply(self, arguments)} );
		}
